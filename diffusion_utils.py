...
def q_sample(y, y_0_hat, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, noise=None):
    ...
    y_t = sqrt_alpha_bar_t * y + (1 - sqrt_alpha_bar_t) * y_0_hat.squeeze() + sqrt_one_minus_alpha_bar_t * noise
    
    #print("y_t shape:", y_t.shape, "y shape:", y.shape, "y_0_hat shape:", y_0_hat.shape, "noise shape:", noise.shape)
...
def compute_kernel(x, y):
    ...
    y_size = y.size(0)
    if len(x.shape) == 1:
        x = x.unsqueeze(1)  # Add an extra dimension to x if it is one-dimensional
    if len(y.shape) == 1:
        y = y.unsqueeze(1)  # Add an extra dimension to y if it is one-dimensional
    dim = x.size(1)
    x = x.unsqueeze(1)  # (x_size, 1, dim)
    y = y.unsqueeze(0)  # (1, y_size, dim)
    tiled_x = x.expand(x_size, y_size, dim)
...
def compute_mmd(x, y):
    '''
    function to compute the maximum mean discrepancy (MMD) between two samples
    ''' 
