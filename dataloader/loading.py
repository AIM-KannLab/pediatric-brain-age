...
class BrainsDataset(Dataset):
    def __init__(self, data_list, std, train=True):
        self.trainsize = (224,224)#(256,256)#(224,224)
        self.train = train
        tr_dl = pd.read_csv(data_list, header=0)
        self.data_list = tr_dl
        self.size = len(self.data_list)
        self.std_dev = std
    ...
    def __getitem__(self, index):
        data_pac = self.data_list.iloc[index]
        img_path = data_pac['dir']
        img = Image.open(img_path).convert('RGB')
        img_torch = self.transform_center(img)
        label = int(data_pac['label'])/self.std_dev #/8.86 # std dev
        return img_torch, label, img_path
